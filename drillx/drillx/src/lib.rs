...
pub fn hash_with_memory(
    ...
    challenge: &[u8; 32],
    nonce: &[u8; 8],
) -> Result<Vec<Hash>, DrillxError> {
    let digest_array = digest_with_memory(memory, challenge, nonce)?;
    Ok(digest_array
        .into_par_iter()
        .map(|digest| Hash {
            d: digest,
...
fn digest_with_memory(
    ...
    Ok(solutions
        .into_par_iter()
        .map(|solution| solution.to_bytes())
        .collect())
}
...
impl Hash {
    ...
    pub fn is_valid(&self, challenge: &[u8; 32], nonce: &[u8; 8]) -> bool {
        is_valid_digest(challenge, &nonce, &self.d)
    }
}
